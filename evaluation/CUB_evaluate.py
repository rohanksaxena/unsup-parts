...
dataset_root='/mnt/nfs-students/unsupervised-parts/unsup-parts/data'
...
def get_nmi_inputs(data_loader, model, num_parts, is_scops):
    ...
    for i, (input_raw, fg_mask_raw, _, landmarks_raw, bbox_raw) in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if i == 1000:
                data = inputs.permute(0,2,3,1).cpu().numpy().squeeze()
                norm = plt.Normalize(vmin=data.min(), vmax=data.max())
                image = norm(data)
                plt.imsave(f'{i}_1.jpg', image)
                plt.imshow(fg_masks.cpu().numpy().squeeze()*50)
                data = fg_masks.cpu().numpy().squeeze()*50
                norm = plt.Normalize(vmin=data.min(), vmax=data.max())
                image = norm(data)
                plt.imsave(f'{i}_2.jpg', image)
                plt.imshow(part_name_mat_w_bg.argmax(1).cpu().numpy().squeeze())
    #             cv2.imwrite(f'{i}_3.jpg', part_name_mat_w_bg.argmax(1).cpu().numpy().squeeze())
    # #                 plt.show()
    # #                 print(landmarks_full.shape)
                plt.scatter(landmarks_full[0, :, 1].detach().cpu(), landmarks_full[0, :, 2].detach().cpu(), color='red')
                plt.axis('off')
                plt.savefig(f'{i}_3', bbox_inches='tight')
    #             plt.show()
...
if __name__ == '__main__':
    is_scops = False
    path = '/mnt/nfs-students/unsupervised-parts/unsup-parts/checkpoints/CUB/model_60000.pth'
    nmi1, ari1 = eval_all(path=path, is_full=False, is_scops=is_scops, only_three=False, req_label=None,
                          regress_landmark=False)
    nmi2, ari2 = eval_all(path=path, is_full=True, is_scops=is_scops, only_three=False, req_label=None,
                          regress_landmark=False)
